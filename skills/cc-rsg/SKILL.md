...
1. **仕様書スケルトンの生成**
   - 確定したテンプレートに基づき、`drafts/`配下に空の章ファイルを作成する。
   - 各章ファイル冒頭に「この章で扱う内容」のメタコメントを置く。
...
1. **章ドラフトの統合**
   - `drafts/`配下の各章を結合し、テンプレートで定義された順序で並べる。
   - 各章ファイル冒頭のメタコメントは除去する。

2. **トレーサビリティ表の生成**
   - 仕様書の各章・節と、参照したソースコードの対応表を別ファイルとして生成する。
   - 形式例:
...
3. **「未確定事項」章の生成**
   - `abandoned`ステータスの疑問をまとめて「未確定事項」章として独立した章を生成する。
   - 各未確定事項について「なぜ確定できなかったか」「現時点でどこまで推測したか」「将来確定するために何が必要か」を記載する。

4. **メタデータの添付**
   - 生成日時、対象コードベースのコミットハッシュ(取得可能なら)、Phase 0で確定したゴール定義、テンプレート選定結果、を仕様書冒頭または別ファイルとして添付する。
...
### このフェーズで気を付けること
- 「未確定事項」章を省略してはならない。仕様書の信頼性を担保する根幹である。
- メタデータを添付しないと、後で「いつ、どのバージョンのコードから生成したか」が不明になる。
